 ...
 * | sku         | name              | price | tax_class_id  | url_key        | qty   |is_in_stock |backorders|website_names          |
 * | 678.678.679 | Product Name      | 5.00  | 11            | simple-product | 100   |1           |0         |Name1,Name2            |
 * And the following bundle products exist:
 * |sku_type    |sku           | name               |url_key              |price_type|was_price_presented|is_available_for_backorder|website_names|
 * |1           |BUNDLE-FIXTURE| Bundle Fixture     |bundle-fixture       |0         |1                  |0                         |Name1, Name2 |
...
final class BundleProduct implements FixtureInterface
{
    ...
    /**
     * @param $bundleModelFactory \Closure optional
     */
    public function __construct($bundleModelFactory = null)
    {
        $this->modelFactory = $bundleModelFactory ?: $this->defaultModelFactory();
    }
    ...
    {
        $bundleFactory = $this->modelFactory;
        $this->bundle  = $bundleFactory();
    ...
    public function delete($identifier)
    {
        ...
        $this->mageUnregisterBundle(self::REGISTRY_FIXTURE_PREFIX . $sku);
        unset(self::$registeredBundles[static::REGISTRY_FIXTURE_PREFIX . $sku]);
    }
    ...
    private function getWebsiteIds()
    {
        ...
        foreach (\Mage::getModel('core/website')->getCollection() as $website) {
            if (is_array($this->websiteNames) && count($this->websiteNames) > 0) {
                foreach ($this->websiteNames as $websiteName) {
                    if ($website->getName() == trim($websiteName)) {
                        $ids[] = $website->getId();
                        break;
                    }
                }
